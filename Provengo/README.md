...
⚠️ NOTE: In the below listings, we assume that `provengo` is in the system's PATH variable, and that `moodle` is the (relative/absolute) path to this directory.
...
Sample 10 scenarios into a file. The scenarios are stored in a file called `samples.json` (this can be changed using the `-o`/`--output-file` switch).
    ...
    provengo sample --delete-previous --sample-size 10 moodle
...
**Requires running `sample` first** (the previous command)**.**
    ...
    provengo ensemble --suite-size 5 moodle
